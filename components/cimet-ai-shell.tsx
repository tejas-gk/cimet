...
import { BarChart3Icon, BotIcon, LayoutGridIcon, PhoneCallIcon, ShieldCheckIcon } from "lucide-react"
...
import { providerConfig } from "@/lib/cimet-ai-adapters"
        ...
        <SidebarHeader className="border-b border-[#27272a] p-3">
          <div className="flex h-9 items-center gap-2 rounded-md px-2">
            ...
            <div className="min-w-0 group-data-[collapsible=icon]:hidden">
              ...
              <div className="truncate text-xs text-zinc-500">Voice + Quality automation</div>
            </div>
          </div>
        </SidebarHeader>
        <SidebarContent>
                ...
                {navItems.map((item) => {
                  const isActive =
                    item.href === "/"
                      ? pathname === "/"
                      : item.href === "/auditor"
                        ? pathname === "/auditor" || /^\/auditor\/[^/]+$/.test(pathname)
          ...
          <SidebarGroup className="group-data-[collapsible=icon]:hidden">
            ...
            <SidebarGroupContent className="space-y-2 px-2 text-xs text-zinc-400">
              <div className="rounded-md border border-[#242427] bg-[#111113] p-2">
                <div className="mb-1 font-medium text-zinc-100">Plug-and-play mode</div>
                <div>Voice: {providerConfig.voice.active}</div>
                <div>Transcription: {providerConfig.transcription.active}</div>
                <div>LLM: {providerConfig.llm.active}</div>
              </div>
              <Badge variant="outline" className="border-[#34363a] bg-[#111113] text-zinc-300">
                Free local mock enabled
              </Badge>
            </SidebarGroupContent>
          </SidebarGroup>
        </SidebarContent>
      ...
      <SidebarInset className="min-w-0 bg-[#080808] text-white">
        <div className="flex h-12 shrink-0 items-center gap-2 border-b border-[#27272a] px-3 sm:px-4">
          ...
          <div className="min-w-0 flex-1">
            <h1 className="truncate text-sm font-semibold">CIMET AI automation console</h1>
          </div>

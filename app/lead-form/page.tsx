...
import { ENERGY_PLANS, ENERGY_STATES, FUEL_TYPES, leadProgress, loadLeads, RETAILERS, USAGE_TIERS, YES_NO } from "@/lib/leads-store"
...
const stepTitles = ["Contact details", "Energy details", "Plan & market", "Message & review"]
...
export default function LeadFormPage() {
  ...
  React.useEffect(() => {
    ...
    const all = loadLeads()
    const requested = requestedId ? all.find((lead) => lead.id === requestedId) ?? null : null
    const draft =
      requested ??
      all
        ...
        <div className="mx-auto grid max-w-2xl gap-5">
          <div className="rounded-xl border border-emerald-500/30 bg-emerald-500/5 p-6">
            ...
            <p className="mt-2 text-sm text-zinc-400">
              {form.name ? `Thanks, ${form.name}. ` : "Thanks — "}Your lead has been saved and is now visible in the leads table.
            </p>
            <div className="mt-4 flex flex-wrap gap-2">
              <Button size="sm" asChild>
                <Link href="/leads">View in Leads</Link>
              </Button>
              <Button size="sm" variant="outline" className="border-[#34363a] bg-[#111113] text-white" onClick={resetForm}>
                Start another lead
              </Button>
            </div>
          </div>
    ...
    <CimetAiShell>
      <div className="mx-auto grid max-w-3xl gap-5">
        <div className="grid gap-2">
          ...
          <h2 className="text-2xl font-semibold tracking-tight">Tell us about your energy needs</h2>
          <p className="text-sm text-zinc-400">
            Everything is saved as you type. You can leave halfway and come back to continue later.
          </p>
        </div>
        ...
        {resumed ? (
          <div className="flex items-center justify-between gap-3 rounded-xl border border-sky-500/30 bg-sky-500/10 p-4">
            <div className="text-sm text-sky-100">
              Editing the draft you started on {activeLead ? formatTime(activeLead.createdAt) : "earlier"}.
            </div>
            <Button size="xs" variant="outline" className="border-sky-500/40 bg-sky-500/10 text-sky-100" onClick={resetForm}>
              Start fresh
            </Button>
          </div>
        ) : null}
          ...
          <div className="flex items-center justify-between gap-3">
                ...
                >
                  <span
                    className={`flex size-5 items-center justify-center rounded-full text-xs font-medium ${
                      index < step ? "bg-emerald-500 text-black" : index === step ? "bg-emerald-500/20 text-emerald-200" : "bg-[#222226] text-zinc-500"
                    }`}
                  >
                    {index < step ? <CheckCircle2Icon className="size-3" /> : index + 1}
                  </span>
            ...
            {autosave ? (
              <div className="flex shrink-0 items-center gap-1.5 text-xs text-zinc-500">
                ...
                Draft saved
                {activeLead ? <span className="hidden text-zinc-600 sm:inline">· {formatTime(activeLead.updatedAt)}</span> : null}
              </div>
            ) : null}
          </div>
        ...
        <div className="rounded-xl border border-[#27272a] bg-[#0b0b0c] p-5">
          {step === 0 ? (
            <div className="grid gap-4">
              <Field icon={<UserIcon className="size-4" />} label="Full name" hint="Required">
                <Input
                  value={form.name}
              ...
              <Field icon={<MailIcon className="size-4" />} label="Email address" hint="Required">
                <Input
                  type="email"
                  value={form.email}
              ...
              <Field icon={<PhoneIcon className="size-4" />} label="Phone number" hint="Required">
                <Input
                  type="tel"
                  value={form.phone}
              ...
              <Field icon={<Building2Icon className="size-4" />} label="Company or organisation" hint="Optional">
                <Input
                  value={form.company}
          ...
          {step === 1 ? (
            <div className="grid gap-4">
              <div className="grid gap-4 sm:grid-cols-2">
                <Field icon={<HomeIcon className="size-4" />} label="Service address" hint="Quality audit">
                  <Input
                    value={form.address}
                ...
                <Field icon={<MapPinIcon className="size-4" />} label="Postcode" hint="Quality audit">
                  <Input
                    value={form.postcode}
              ...
              <div className="grid gap-4 sm:grid-cols-2">
                <Field icon={<CalendarDaysIcon className="size-4" />} label="Date of birth" hint="Quality audit">
                  <Input
                    type="date"
                    value={form.dob}
                ...
                <Field icon={<FuelIcon className="size-4" />} label="Fuel type" hint="Quality audit">
              ...
              <Field icon={<HashIcon className="size-4" />} label="NMI / MIRN" hint="Quality audit">
                <Input
                  value={form.nmiMirn}
                  onChange={(event) => updateField("nmiMirn", event.target.value)}
                  placeholder="4102001234"
                  className="border-[#27272a] bg-[#111113] text-white"
                />
              </Field>
              <div className="grid gap-4 sm:grid-cols-2">
                <Field icon={<HeartHandshakeIcon className="size-4" />} label="Concession card holder" hint="Quality audit">
                  <Select value={form.concession} onValueChange={(value) => updateField("concession", value)}>
                    <SelectTrigger className="w-full border-[#27272a] bg-[#111113] text-white">
                ...
                <Field icon={<HeartHandshakeIcon className="size-4" />} label="Life support required" hint="Quality audit">
                  <Select value={form.lifeSupport} onValueChange={(value) => updateField("lifeSupport", value)}>
                    <SelectTrigger className="w-full border-[#27272a] bg-[#111113] text-white">
                      <SelectValue placeholder="Select" />
                    </SelectTrigger>
                    <SelectContent>
              ...
              <Field icon={<CalendarDaysIcon className="size-4" />} label="Move-in date" hint="Quality audit">
                <Input
                  type="date"
          ...
          {step === 2 ? (
            <div className="grid gap-4">
              <div className="grid gap-4 sm:grid-cols-2">
                <Field icon={<MapPinIcon className="size-4" />} label="State / market" hint="Optional">
                ...
                <Field icon={<Building2Icon className="size-4" />} label="Retailer" hint="Determines the QA checklist">
                  <Select value={form.retailer} onValueChange={(value) => updateField("retailer", value)}>
                    <SelectTrigger className="w-full border-[#27272a] bg-[#111113] text-white">
              ...
              <div className="grid gap-4 sm:grid-cols-2">
                <Field icon={<ZapIcon className="size-4" />} label="Interested plan" hint="Optional">
                ...
                <Field icon={<ZapIcon className="size-4" />} label="Current monthly bill" hint="Optional">
                  <Select value={form.usage} onValueChange={(value) => updateField("usage", value)}>
                    <SelectTrigger className="w-full border-[#27272a] bg-[#111113] text-white">
          ...
          {step === 3 ? (
            <div className="grid gap-4">
              <Field icon={<FileTextIcon className="size-4" />} label="Message" hint="Optional — anything else we should know">
                <Textarea
                  value={form.message}
              ...
              <div className="rounded-xl border border-[#242427] bg-[#111113] p-4">
                <div className="font-medium text-zinc-100">Review your details</div>
              ...
              <label className="flex items-start gap-3 rounded-xl border border-[#242427] bg-[#111113] p-4">
                ...
                />
                <div>
                  <div className="text-sm font-medium text-zinc-100">Consent to contact</div>
                  <div className="text-xs text-zinc-500">I agree to be contacted about energy plans and offers.</div>
                </div>
              </label>
            </div>
          ) : null}
          ...
          {step === 3 && !canSubmit ? (
            <p className="mt-2 text-right text-xs text-zinc-500">Name, email and phone are required to submit.</p>
          ) : null}
        </div>
      </div>
    </CimetAiShell>
...
function Detail({ label, value }: { label: string; value: string }) {
  return (
    <div>
      ...
      <dd className={value ? "text-zinc-100" : "text-zinc-600"}>{value || "—"}</dd>
    </div>
  )
}
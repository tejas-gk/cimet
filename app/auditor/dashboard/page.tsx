...
export default function DashboardPage() {
  ...
  const byRetailer = audits.reduce<Record<string, number>>((acc, audit) => {
    acc[audit.retailer] =
      (acc[audit.retailer] ?? 0) + (audit.status === "hold" ? 1 : 0)
    return acc
  }, {})
    ...
    <CimetAiShell>
      <div className="mx-auto grid max-w-7xl gap-5">
        <div>
          <Badge
            variant="outline"
            className="mb-2 border-emerald-500/30 bg-emerald-500/10 text-emerald-200"
          >
          ...
          <p className="mt-1 max-w-3xl text-sm text-zinc-400">
            Shows the core business outcomes CIMET asked for: auto-pass rate,
            holds, reviews, common failures and AI/human agreement.
          </p>
        </div>
                ...
                >
                  ...
                  <div className="mt-3 h-2 rounded-full bg-[#222226]">
                    <div
                      ...
                      style={{ width: `${Math.max(8, count * 35)}%` }}
                    />
                  </div>
                </div>
              ))}
...
function Metric({
  ...
  return (
    <div className="rounded-xl border border-[#27272a] bg-[#0d0d0f] p-4">
      <div className="mb-3 flex size-8 items-center justify-center rounded-md bg-white text-black">
        {icon}
      </div>

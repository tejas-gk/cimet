...
import { CheckCircle2Icon, ClockIcon, ExternalLinkIcon, RotateCcwIcon, XCircleIcon } from "lucide-react"
...
export default function AuditDetailPage() {
  ...
  if (!audit) {
    return (
      <CimetAiShell>
        <div className="rounded-xl border border-[#27272a] bg-[#0d0d0f] p-6">Audit not found.</div>
      </CimetAiShell>
    )
  }
      ...
      <div className="mx-auto grid max-w-7xl gap-5 xl:grid-cols-[0.9fr_1.1fr]">
        <div className="grid content-start gap-5">
          <div className="rounded-xl border border-[#27272a] bg-[#0d0d0f] p-5">
            <Badge variant="outline" className="mb-2 border-violet-500/30 bg-violet-500/10 text-violet-200">QA audit</Badge>
            ...
            <div className="mt-4 grid grid-cols-3 gap-3 text-sm">
              ...
              <div className="rounded-lg border border-[#242427] bg-[#111113] p-3"><div className="text-zinc-500">Checks</div><div>{audit.checks.length}</div></div>
            </div>
          </div>

          <div className="rounded-xl border border-[#27272a] bg-[#0b0b0c]">
            <div className="border-b border-[#27272a] p-4 font-medium">Diarized transcript</div>
            <div className="grid max-h-[580px] gap-3 overflow-y-auto p-4">
              {audit.transcript.map((utterance) => (
                <button key={utterance.id} type="button" className="rounded-lg border border-[#242427] bg-[#111113] p-3 text-left transition-colors hover:border-sky-500/40 hover:bg-sky-500/10">
                  <div className="mb-1 flex items-center gap-2 text-xs text-zinc-500"><ClockIcon className="size-3" /> {formatMs(utterance.startMs)} · {utterance.speaker}</div>
        ...
        <div className="grid content-start gap-5">
          <div className="rounded-xl border border-[#27272a] bg-[#0b0b0c]">
            <div className="border-b border-[#27272a] p-4 font-medium">Checklist evidence</div>
            <div className="grid gap-3 p-4">
              {audit.checks.map((check) => (
                <div key={check.id} className="rounded-xl border border-[#242427] bg-[#111113] p-4">
                  <div className="flex flex-wrap items-start justify-between gap-2">
                    <div>
                      <div className="font-medium text-zinc-100">{check.label}</div>
                      <div className="mt-1 text-xs text-zinc-500">{check.type} · {check.critical ? "critical" : "coaching"} · {check.confidence}% confidence</div>
                    </div>
                    <Badge variant="outline" className={verdictStyle(check.verdict)}>{check.verdict === "pass" ? <CheckCircle2Icon className="size-3" /> : check.verdict === "fail" ? <XCircleIcon className="size-3" /> : <RotateCcwIcon className="size-3" />} {check.verdict}</Badge>
                  </div>
                  <p className="mt-3 text-sm text-zinc-300">{check.finding}</p>
                  <div className="mt-3 grid gap-2">
                    {check.evidence.map((evidence) => (
                      <div key={`${check.id}-${evidence.utteranceId}`} className="rounded-lg border border-[#2b2b30] bg-[#0b0b0c] p-3 text-sm">
                        <div className="mb-1 flex items-center gap-2 text-xs text-zinc-500"><ExternalLinkIcon className="size-3" /> {formatMs(evidence.startMs)} - {formatMs(evidence.endMs)}</div>
                        <div className="text-zinc-200">“{evidence.quote}”</div>
                        {evidence.correctValue ? <div className="mt-2 text-xs text-emerald-300">Correct value: {evidence.correctValue}</div> : null}
                      </div>
                    ))}
                  </div>
                </div>
          ...
          <div className="rounded-xl border border-[#27272a] bg-[#0d0d0f] p-4">
            ...
            <p className="mt-1 text-sm text-zinc-500">Disagreement is stored so CIMET can measure AI-vs-human QA agreement.</p>
            <div className="mt-3 grid gap-2">
              <Select value={decision} onValueChange={(value) => setDecision(value as AuditDecision)}>

...
import { ArrowRightIcon, PhoneCallIcon, ShieldAlertIcon } from "lucide-react"
...
export default function CallsPage() {
  const { calls, journeys, placeCall } = useCimetAi()
      ...
      <div className="mx-auto grid max-w-7xl gap-5">
        <div className="grid gap-2">
          ...
          <p className="max-w-3xl text-sm text-zinc-400">
            Mock queue for customers who abandoned comparison. The UI is wired to a local adapter now and can later swap to Vapi, Twilio, LiveKit, Gemini, Deepgram, or a backend API without changing the screen.
          </p>
        </div>
              ...
              <div key={call.id} className="grid gap-3 border-b border-[#202023] p-4 last:border-b-0 lg:grid-cols-[1.4fr_1fr_auto] lg:items-center">
                <div className="min-w-0">
                  <div className="flex flex-wrap items-center gap-2">
                    ...
                    {call.handoff ? <Badge variant="outline" className="border-amber-500/40 text-amber-300"><ShieldAlertIcon className="size-3" /> handoff ready</Badge> : null}
                  </div>
                ...
                <div className="flex gap-2 lg:justify-end">
                  {call.status === "queued" ? (
                    <Button size="sm" onClick={() => placeCall(call.id)}>
                      <PhoneCallIcon className="size-4" /> Simulate call
                    </Button>
                  ) : null}
                  <Button size="sm" variant="outline" className="border-[#34363a] bg-[#111113] text-white" asChild>
                    <Link href={`/calls/${call.id}`}>Open <ArrowRightIcon className="size-4" /></Link>
                  </Button>
                </div>
              </div>
            )

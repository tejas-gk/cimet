 ...
 */
export const GET = handle(async (_req: Request, ctx: RouteContext<"/api/calls/[id]/audit">) => {
  const { id } = await ctx.params
  const db = getDb()
  const callAudit = listAudits(db).find((a) => a.leadId === id) ?? null
  if (!callAudit) return apiError("No audit for this call yet", 404, "not_found")
  return json({ data: callAudit })
})

export const POST = handle(async (_req: Request, ctx: RouteContext<"/api/calls/[id]/audit">) => {
  const { id } = await ctx.params
  const db = getDb()
  try {

  ...
  const focusCellWrapper = React.useCallback(
    (rowIndex: number, columnId: string) => {
      ...
      requestAnimationFrame(() => {
        ...
        // Skip focusing when focus is already inside this cell (e.g. a
        // contentEditable editor that just opened on double click). Refocusing
        // would blur the editor and cancel the edit.
        if (
          document.activeElement === cellWrapperElement ||
          cellWrapperElement.contains(document.activeElement)
        ) {
          releaseFocusGuard()
          return
        }
